...
from gyp.org.thepanday.informatikproject.common import GypError
from gyp.org.thepanday.informatikproject.common import OrderedSet
...
def LoadOneBuildFile(build_file_path, data, aux_data, includes, is_target, check):
    ...
    try:
        ...
        gyp.org.thepanday.informatikproject.common.ExceptionAppend(e, "while reading " + build_file_path)
        raise
    ...
    if "skip_includes" not in build_file_data or not build_file_data["skip_includes"]:
        try:
            ...
            gyp.org.thepanday.informatikproject.common.ExceptionAppend(
...
def LoadTargetBuildFile(
    ...
    if depth:
        ...
        d = gyp.org.thepanday.informatikproject.common.RelativePath(depth, os.path.dirname(build_file_path))
        if d == "":
            variables["DEPTH"] = "."
        else:
    ...
    for included_file in included:
        ...
        included_relative = gyp.org.thepanday.informatikproject.common.RelativePath(
    ...
    if "targets" in build_file_data:
        for target_dict in build_file_data["targets"]:
            ...
            for dependency in target_dict["dependencies"]:
                dependencies.append(
                    gyp.org.thepanday.informatikproject.common.ResolveTarget(build_file_path, dependency, None)[0]
                )
    ...
    for match_group in matches:
        ...
        if file_list:
            ...
            if not generator_filelist_paths:
                ...
                if os.path.isabs(build_file_dir):
                    ...
                    rel_build_file_dir = gyp.org.thepanday.informatikproject.common.RelativePath(
                ...
                gyp.org.thepanday.informatikproject.common.EnsureDirExists(path)

            replacement = gyp.org.thepanday.informatikproject.common.RelativePath(path, build_file_dir)
            f = gyp.org.thepanday.informatikproject.common.WriteOnDiff(path)
        ...
        if expand_to_list:
            ...
            if type(replacement) is list:
                ...
                # routine.
                encoded_replacement = gyp.org.thepanday.informatikproject.common.EncodePOSIXShellList(replacement)
            else:
  ...
  """
    ...
    for build_file in data["target_build_files"]:
        for target in data[build_file].get("targets", []):
            target_name = gyp.org.thepanday.informatikproject.common.QualifiedTarget(
...
def QualifyDependencies(targets):
  ...
  """
    ...
    for target, target_dict in targets.items():
        target_build_file = gyp.org.thepanday.informatikproject.common.BuildFile(target)
        toolset = target_dict["toolset"]
        for dependency_key in all_dependency_sections:
            dependencies = target_dict.get(dependency_key, [])
            for index, dep in enumerate(dependencies):
                dep_file, dep_target, dep_toolset = gyp.org.thepanday.informatikproject.common.ResolveTarget(
                    target_build_file, dep, toolset
                )
                if not multiple_toolsets:
                    # Ignore toolset specification in the dependency if it is specified.
                    dep_toolset = toolset
                dependency = gyp.org.thepanday.informatikproject.common.QualifiedTarget(
        ...
        for dependency_key in dependency_sections:
            ...
            while index < len(dependencies):
                (
                    ...
                    dependency_target,
                    dependency_toolset,
                ) = gyp.org.thepanday.informatikproject.common.ParseQualifiedTarget(dependencies[index])
...
def VerifyNoGYPFileCircularDependencies(targets):
    ...
    for target in targets:
        build_file = gyp.org.thepanday.informatikproject.common.BuildFile(target)
    ...
    for target, spec in targets.items():
        ...
        for dependency in target_dependencies:
            try:
                dependency_build_file = gyp.org.thepanday.informatikproject.common.BuildFile(dependency)
            except GypError as e:
                gyp.org.thepanday.informatikproject.common.ExceptionAppend(
...
def MakePathRelative(to_file, fro_file, item):
    ...
    if to_file == fro_file or exception_re.match(item):
        ...
        ret = os.path.normpath(
            os.path.join(
                gyp.org.thepanday.informatikproject.common.RelativePath(
...
def PruneUnwantedTargets(targets, flat_list, dependency_nodes, root_targets, data):
    ...
    for target in root_targets:
        target = target.strip()
        qualified_targets = gyp.org.thepanday.informatikproject.common.FindQualifiedTargets(target, flat_list)
    ...
    for build_file in data["target_build_files"]:
        ...
        for target in data[build_file]["targets"]:
            qualified_name = gyp.org.thepanday.informatikproject.common.QualifiedTarget(
                build_file, target["target_name"], target["toolset"]
            )
...
def Load(
    ...
    if parallel:
        ...
        for build_file in build_files:
            try:
                ...
                gyp.org.thepanday.informatikproject.common.ExceptionAppend(e, "while trying to load %s" % build_file)
                raise
    ...
    # some may, and it seems best to build the list in a org.thepanday.informatikproject.common spot.
