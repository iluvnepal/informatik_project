...
def CalculateVariables(default_variables, params):
    ...
    default_variables.setdefault("OS", gyp.org.thepanday.informatikproject.common.GetFlavor(params))
...
def WriteActions(target_name, actions, extra_sources, extra_deps, path_to_gyp, output):
  ...
  """
    for action in actions:
        ...
        output.write(gyp.org.thepanday.informatikproject.common.EncodePOSIXShellList(action["action"]))
        output.write("\n")
  ...
  """
    for rule in rules:
        ...
        for count, rule_source in enumerate(rule.get("rule_sources", [])):
            ...
            output.write("  COMMAND ")
            output.write(gyp.org.thepanday.informatikproject.common.EncodePOSIXShellList(rule["action"]))
            output.write("\n")
...
def CreateCMakeTargetBaseName(qualified_target):
    ...
    _, gyp_target_name, gyp_target_toolset = gyp.org.thepanday.informatikproject.common.ParseQualifiedTarget(
        qualified_target
    )
...
def CreateCMakeTargetFullName(qualified_target):
    ...
    gyp_file, gyp_target_name, gyp_target_toolset = gyp.org.thepanday.informatikproject.common.ParseQualifiedTarget(
        qualified_target
    )
...
def WriteTarget(
    ...
    gyp_file, _, _ = gyp.org.thepanday.informatikproject.common.ParseQualifiedTarget(qualified_target)
    rel_gyp_file = gyp.org.thepanday.informatikproject.common.RelativePath(gyp_file, options.toplevel_dir)
    rel_gyp_dir = os.path.dirname(rel_gyp_file)

    # Relative path from build dir to top dir.
    build_to_top = gyp.org.thepanday.informatikproject.common.InvertRelativePath(build_dir, options.toplevel_dir)
    ...
    if static_deps or shared_deps or other_deps:
        for extra_dep in extra_deps:
            ...
            for deps in (static_deps, shared_deps, other_deps):
                for dep in gyp.org.thepanday.informatikproject.common.uniquer(deps):
                    output.write("  ")
                    output.write(dep)
                    output.write("\n")
            output.write(")\n")
    ...
    if other_deps or (not linkable and (static_deps or shared_deps)):
        ...
        output.write(cmake_target_name)
        output.write("\n")
        for dep in gyp.org.thepanday.informatikproject.common.uniquer(other_deps):
            output.write("  ")
            output.write(dep)
            output.write("\n")
        if not linkable:
            for deps in (static_deps, shared_deps):
                for lib_dep in gyp.org.thepanday.informatikproject.common.uniquer(deps):
                    output.write("  ")
    ...
    if linkable:
        ...
        if external_libs or static_deps or shared_deps:
            ...
            if static_deps:
                ...
                for dep in gyp.org.thepanday.informatikproject.common.uniquer(static_deps):
                    output.write("  ")
                    output.write(dep)
                    output.write("\n")
                if write_group:
            ...
            if shared_deps:
                for dep in gyp.org.thepanday.informatikproject.common.uniquer(shared_deps):
                    output.write("  ")
                    output.write(dep)
                    output.write("\n")
            if external_libs:
                for lib in gyp.org.thepanday.informatikproject.common.uniquer(external_libs):
                    output.write('  "')
...
def GenerateOutputForConfig(target_list, target_dicts, data, params, config_to_use):
    ...
    gyp.org.thepanday.informatikproject.common.EnsureDirExists(output_file)

    output = open(output_file, "w")
    output.write("cmake_minimum_required(VERSION 2.8.8 FATAL_ERROR)\n")
    output.write("cmake_policy(VERSION 2.8.8)\n")

    gyp_file, project_target, _ = gyp.org.thepanday.informatikproject.common.ParseQualifiedTarget(target_list[-1])
    ...
    ar = gyp.org.thepanday.informatikproject.common.GetEnvironFallback(["AR_target", "AR"], ar)
    cc = gyp.org.thepanday.informatikproject.common.GetEnvironFallback(["CC_target", "CC"], cc)
    cxx = gyp.org.thepanday.informatikproject.common.GetEnvironFallback(["CXX_target", "CXX"], cxx)
    ...
    for build_file in params["build_files"]:
        for qualified_target in gyp.org.thepanday.informatikproject.common.AllTargets(
