...
public class UnderstatDataParser {
    ...
    public ScrapedPageContainer scrapeAllMatchesFromAllLeaguesAndYearsToTeamsDataContainer() {
        final ScrapedPageContainer scrapedPageContainer = new ScrapedPageContainer(new HashMap<>());
        LOGGER.info("Fetching data from:");
        for (String pageUrl : this.getPageUrls()) {
            LOGGER.info(pageUrl);
            final ScrapedPageContainer containerToAdd = this.pageToTeamsDataContainer(pageUrl);
            scrapedPageContainer.addToEntries(containerToAdd);
        }
        LOGGER.info("Fetching data completed.");
        MatchDataUtility.appendJsonDataToFile(mJsonData.toString(), MatchDataUtility.DEFAULT_JSON_DATA_FILE);
        return scrapedPageContainer;
    }
    ...
    protected ScrapedPageContainer pageToTeamsDataContainer(String url) {
        ...
        return MatchDataUtility.jsonToTeamsDataContainer(dataAsJsonString);
    }
    ...
    public void populateLeagueTeamsMap() {
        ...
        for (String league : LEAGUES) {
            ...
            final ScrapedPageContainer yearTeamData = pageToTeamsDataContainer(pageUrl);
